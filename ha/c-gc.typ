...
#proof[
  ...
  $
  Take any $h : A -> triv(B)$, then $h$ is a group homomorphism $A -> B$ such that for all $g in G$ and $a in A$, 
  $ h (g dot a) = g dot h(a) = h(a) <=> h(g dot  a - a) = 0 <=> g dot a - a in Ker(h), $
...
#proof[
  ...
  Define the group homomorphism $sigma : fJ  &-> G over [G, G]$ linearly expanded by
  $
    n(g - 1) &|-> g^n [G, G ].
  $
...
#corollary[
  We have
  $
    & H_n lr((C_m , bb(Z))) eq cases(
      bb(Z) quad & "if " n eq 0\,, 
      bb(Z) slash m  quad & "if " n gt.eq 1 "is odd" comma, 
      0 quad & "else. ")\
    & H^n lr((C_m , bb(Z))) eq cases(
      bb(Z) quad& "if " n eq 0\,, 
      bb(Z) slash m quad & "if " n gt.eq 2 "is even,", 
      0  quad & "else. ")
  $]
...
#corollary[
  ...
  $ H_0 lr((G , bb(Z))) tilde.equiv H^0 lr((G , bb(Z))) tilde.equiv bb(Z), \
    H_1 lr((G , bb(Z))) tilde.equiv xor.big_(x in X) bb(Z), \ 
    H^1 lr((G , bb(Z))) tilde.equiv product_(x in X) bb(Z).
  $
]
...
#definition[
  ...
  $
    d_0 ([g_1 lr(|dots.h|) g_n]) & eq g_1 [g_2 lr(|dots.h|) g_n] comma \
    d_i lr(([g_1 lr(|dots.h|) g_n])) & eq [g_1 lr(|dots.h|) g_i g_(i plus 1) lr(|dots.h|) g_n] quad upright(" for ") 1 lt.eq i lt.eq n minus 1 comma\
    d_n lr(([g_1 lr(|dots.h|) g_n])) & eq [g_1 lr(|dots.h|) g_(n minus 1)] dot
  $
