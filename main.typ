...
// ]
// #proof[
  // In view of #thmref(<j-ker>)[Lemma], we only have to verify that $IM(ZZ C_m ->^N ZZ C_m) = ZZ dot N$. Take $a = sum_(j = 0)^(m-1) n_j sigma^j in ZZ C_m$, we compute 
  // $
...
#proof[
  This is obtained by splicing the sequences from #thmref(<ses-norm>)[Corollary] and #thmref(<ses-cyclic>)[Lemma] together.
]
