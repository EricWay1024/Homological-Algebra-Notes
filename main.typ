...
#theorem[
  If $ 0 -> A_cx rgt(f) B_cx rgt(g) Ccx -> 0$ is a short exact sequence of chain complexes, there is a natural map (for each $n$) $ diff: H_n (C) -> H_(n-1) (A) $ (which we call the *connecting homomorphism* or *connecting map*) making 
...
#definition[
  Since $Hom(A)(A, -): cA -> Ab$ is left exact, if there are enough injectives, we can define 
  $ Ext_cA^i (A, B):= R^i Hom(A)(A, -)(B) = H^i (Hom_(A) (A, I^cx)) $
  where $B -> I^cx$.
]
#proposition[
...
#definition[
  The *mapping cylinder* $M_f$ is the quotient
...
#definition[
  The *mapping cone* $C_f$ is the quotient space of the mapping cylinder $(X times I) union.sq_f Y$ with respect to the equivalence relation $(x, 0) tilde (x', 0)$ for all $x, x' in X$. That is, the end of $X times I$ that is not glued to $Y$ is identified as a point.
  #align(center,image("imgs/Mapping_cone.svg",width:30%))
]

On the level of complexes, suppose $f : B_cx -> Ccx$. Define $cone(f)_n = B_(n+1) plus.circle C_n$ and $ d(b, c) = (-d(b), d(c) - f(b)) $

We could also write in the form of a matrix:
$
mat(-d_b, 0; -f, d_c) : vec(B_(n-1), xor, C_n) -> vec(B_(n-2), xor, C_(n-1))
$
There is a #sest:
$ ses(C, cone(f), B[-1]) $
where the first map is $c mapsto (0, c)$ and the second map $delta : (b, c) |-> -b$. We get the #lest in homology:
...
#proof[
  Let $b in B_n$ be a cycle. We can lift it to $(-b, 0)$ in $cone(f)$. Apply diff to get $(d b, f b) = (0, f b)$. Thus $diff[b] = [f b] = f_ast b$.
]
#corollary[
...
#definition[
  $cyl(f)_n = B_n xor B_(n-1) xor C_n$. The matrix is 
  $
...
#definition[
  A *double complex* (or *bicomplex*) in $cA$ is a family ${C_p, q}$ of objects in $cA$ with maps $d^h : C_(p, q) -> C_(p-1, q)$ and $d^v : C_{p, q} -> C_{p, q-1}$ such that $(d^h)^2 = (d^v)^2 = 0$ and $d^v d^h + d^h d^v = 0$ (anti-commute!).
]
#remark[
...
We have that $d^2 = 0$. (which is why we define double complex in the anti-commuting way)
]
#lemma[
